...
/**
 * Implements Runnable interface to run Discord Rich Presence functionality. Downloads and sets up the Discord SDK if
 * needed.
 *
 * @author SpigotRCE
 * @since 2.17
 */
@SuppressWarnings("BusyWait") public class RPC {
  public Core CORE;
  ...
  /**
   * Updates the large image used in the Discord Rich Presence at runtime. Safe to call from external threads.
   *
  ...
  /**
   * Entry point for the Discord RPC thread. Continuously updates presence based on client state.
   */
  public void run() {
    try {
      installDiscordSDK();
    ...
    try {
      File sdkFile = new File(MinecraftClient.getInstance().runDirectory, "paradise/discord/discord_game_sdk.dll");
      Core.init(sdkFile);
    } catch (Exception e) {
