package net.paradise_client.chatroom.server.database;
...
public class MySQLDatabase {
  ...
  private void connect() throws SQLException {
    ...
    this.connection =
      DriverManager.getConnection("jdbc:mysql://" + model.host() + "/" + model.name() + model.parameters(),
        model.username(),
        model.password());
  }
  ...
  public void insertUser(UserModel user) throws SQLException {
    connect();
    String query =
      "INSERT INTO users (discordID, uuid, dateOfRegistration, username, email, token, verified) VALUES (?, ?, ?, ?, ?, ?, ?)";
  ...
  public UserModel getUser(long discordID) throws SQLException {
    ...
    try (PreparedStatement pstmt = connection.prepareStatement(query)) {
      ...
      try (ResultSet rs = pstmt.executeQuery()) {
        if (rs.next()) {
          return new UserModel(rs.getLong("discordID"),
  ...
  public List<UserModel> getAllUsers() throws SQLException {
    ...
    try (Statement stmt = connection.createStatement(); ResultSet rs = stmt.executeQuery(query)) {
      while (rs.next()) {
        users.add(new UserModel(rs.getLong("discordID"),
          UUID.fromString(rs.getString("uuid")),
          rs.getDate("dateOfRegistration"),
          rs.getString("username"),
          rs.getString("email"),
  ...
  public void updateUser(UserModel user) throws SQLException {
    connect();
    String query =
      "UPDATE users SET uuid = ?, dateOfRegistration = ?, username = ?, email = ?, token = ?, verified = ? WHERE discordID = ?";
    try (PreparedStatement pstmt = connection.prepareStatement(query)) {
