...
public class ServerPacketHandler extends AbstractPacketHandler {
  ...
  @Override public void handle(HandshakePacket packet) throws Exception {
    if (isAuthenticated) {
      throw new IllegalStateException("User already authenticated");
    }
    if (packet.getPvn() != Version.PROTOCOL_VERSION) {
      PacketRegistry.sendPacket(new HandshakeResponsePacket(new UserModel().withUsername("Invalid client version!"),
        false), channel);
      channel.close();
      return;
    }
    ...
    if (ChatRoomServer.lastConnectionTime.containsKey(hostname)) {
      if (ChatRoomServer.lastConnectionTime.get(hostname) + Main.CONFIG.getServer().connectionThrottle() >
        System.currentTimeMillis()) {
        PacketRegistry.sendPacket(new HandshakeResponsePacket(new UserModel().withUsername("Connection throttled!"),
          false), channel);
        channel.close();
        return;
      }
    ...
    if (!isAuthenticated) {
      PacketRegistry.sendPacket(new HandshakeResponsePacket(new UserModel(), false), channel);
      channel.close();
      return;
    }
    ...
    if (ChatRoomServer.onlineUsers.contains(userModel)) {
      PacketRegistry.sendPacket(new HandshakeResponsePacket(new UserModel().withUsername(
        "You are already connected the server."), false), channel);
      userModel = null;
      channel.close();
      return;
    }
    PacketRegistry.sendPacket(new HandshakeResponsePacket(userModel, true), channel);
  ...
  @Override public void handle(DisconnectPacket packet) throws Exception {
    channel.close();
  }

  @Override public void handle(MessagePacket packet) throws Exception {
    if (!isAuthenticated) {
      throw new IllegalStateException("User not authenticated");
    }

    if (lastMessage + Main.CONFIG.getServer().messageCooldown() > System.currentTimeMillis()) {
      PacketRegistry.sendPacket(new MessagePacket("Do not spam messages!"), channel);
      return;
    }
    lastMessage = System.currentTimeMillis();
    if (packet.getMessage().length() > Main.CONFIG.getServer().maxMessageCharacters()) {
      PacketRegistry.sendPacket(new MessagePacket("Message is too long!"), channel);
      return;
    }

    ChatRoomServer.broadcastMessage(userModel.username() + ">>" + packet.getMessage());
  }
}
