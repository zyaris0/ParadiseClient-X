...
public class AddonLoader {
  ...
  public static void loadAddons() {
    FabricLoader.getInstance().getEntrypointContainers("paradise", ParadiseAddon.class).forEach(container -> {
      try {
        ParadiseAddon addon = container.getEntrypoint();
        addon.name = container.getProvider().getMetadata().getName();
        addon.authors =
          container.getProvider().getMetadata().getAuthors().stream().map(Person::getName).toArray(String[]::new);
